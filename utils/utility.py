...
def process_skeleton(bone_names, bones_map):
    """
    function to process a skeleton
    ...
    bones_index_map = {bone: bone_names.index(bone) for bone in bone_names}
...
def plot_single_animation(ax, x, y, z, lines_map, t, n_markers):
    ...
    # Plot connections
    if len(lines_map) > 0:
        for i in range(len(x[:n_markers, t])):
...
def create_single_plot(x,y,z, bone_names, bones_map,  n_frames, n_markers, rot):
    """
    Wrapper function to create and display multiple subplots.
